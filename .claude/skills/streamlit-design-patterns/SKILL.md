...
## Design System — каркас страницы

Все страницы дашборда следуют единому визуальному стилю.
Отступать от него нельзя — это нарушение дизайн-кода.

### Заголовок страницы

```python
st.markdown("## :primary[:material/<icon>:] Название страницы")
st.caption("Одна строка — описание назначения страницы")
```

- Уровень: H2 (`##`)
- Иконка: Material Design, всегда `:primary:` (оранжевый #FF9800)
- Подзаголовок: `st.caption()` — серый, меньший шрифт

### Блок / секция

```python
with st.container(border=True):
    st.markdown("#### :primary[:material/<icon>:] Название блока")
    # содержимое
```

- Уровень заголовка внутри блока: H4 (`####`)
- Иконка блока: Material Design, всегда `:primary:`
- Граница блока: `border=True` — тонкая рамка, никакого кастомного CSS

### Правила иконок

| Место | Уровень | Размер (контекст) |
|-------|---------|-------------------|
| Заголовок страницы | H2 | ~32px |
| Заголовок блока | H4 | ~20px |
| Кнопки | `st.button(":material/icon: Текст")` | ~18px |

- Иконки выбираются семантически (inventory для каталога, analytics для аналитики и т.д.)
- Цвет иконок в заголовках: всегда `:primary[...]`
- В кнопках: без `:primary:` обёртки — встроены напрямую в текст кнопки
